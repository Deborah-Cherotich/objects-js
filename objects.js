...
// Write a function that takes an array of objects, where each 
...
//  with the category names as keys and the arrays of products as values. const products = [
...
function categoryProperty(){
     let arrange= Object.groupBy(product,({categories}) => categories)
    console.log(arrange);
}
categoryProperty();

// Given an array of objects, where each object represents a student 
// with a name and an array of scores, write a function that returns a new
//  array containing the names of all students whose average 
// score is greater than or equal to 85. const students = [
...
function greatAvaragescore(students){
    const averageScore= (score)=>{
        return score.reduce((sum,score)=>sum+score,0)/score.length;
    }
    const ans= students.filter(students=>
        averageScore(students.score)>=85);

        const resl = ans.map(students=>students.name);
        console.log(resl);
};
greatAvaragescore(students)
...
// to the object called age. The age method should return the current age of the car
